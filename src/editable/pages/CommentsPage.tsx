  ...
  return (
    <EditableSiteShell>
      <main className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
        <section className="rounded-2xl border border-[var(--editable-border)] bg-white p-6 shadow-[0_12px_40px_rgba(0,0,0,0.08)] sm:p-8">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="inline-flex items-center gap-2 text-xs font-extrabold uppercase tracking-[0.24em] text-[#72796c]">
              ...
              <p className="mt-4 max-w-2xl text-sm leading-7 text-[#72796c]">
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="relative w-full sm:max-w-md">
              <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#72796c]" />
              <input
                value={query}
                onChange={(event) => {
                ...
                className="h-11 w-full rounded-lg border border-[var(--editable-border)] bg-white pl-9 pr-3 text-sm outline-none"
              />
            </div>
            <p className="text-sm text-[#72796c]">
        ...
        {visibleComments.length ? (
              ...
              <article key={`${item.articleSlug}-${item.id}`} className="rounded-2xl border border-[var(--editable-border)] bg-white p-5 shadow-sm">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-[#72796c]">{formatDate(item.createdAt)}</p>
                  </div>
                  {item.articleSlug ? (
                    <Link href={`/article/${item.articleSlug}`} className="text-sm font-black text-[#ff4a1f] underline-offset-4 hover:underline">
                      Open article
                    </Link>
                  ) : null}
                </div>
                {item.articleTitle ? <p className="mt-4 text-sm font-extrabold">{item.articleTitle}</p> : null}
                <p className="mt-3 text-sm leading-7 text-[#566052]">{item.comment}</p>
              </article>
            ))}
          </section>
        ) : (
          <section className="mt-8 rounded-2xl border border-dashed border-[var(--editable-border)] bg-white/70 p-8 text-center">
            ...
            <p className="mt-2 text-sm text-[#72796c]">Add a comment on any article page and it will appear here.</p>
          </section>
        )}

        {filtered.length > COMMENTS_PER_PAGE ? (
          <div className="mt-8 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[var(--editable-border)] bg-white p-4 text-sm text-[#72796c]">
